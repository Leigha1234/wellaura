...
const getIconForHabit = (habitName) => { const name = habitName.toLowerCase(); for (const keyword in KEYWORD_ICON_MAP) { if (name.includes(keyword)) { return KEYWORD_ICON_MAP[keyword]; } } return '🎯'; };const getStartOfWeek = (date) => new Date(date.setDate(date.getDate() - date.getDay()));
...
const HabitModal = ({ isVisible, onClose, onSave, habitToEdit, theme, styles }) => {
    ...
    const handleSave = async () => {
      ...
      const baseData: Omit<Habit, 'type' | 'goal'> = { id: habitToEdit ? habitToEdit.id : Date.now().toString(), name, icon: finalIcon, color, reminderTime: reminderTimeString, notificationId: habitToEdit?.notificationId || null, history: habitToEdit?.history || {}, addToCalendar: reminderDate ? addToCalendar : false, };
      ...
      else if (type === 'quit_habit') { habitData = { ...baseData, type: 'quit_habit', reminderTime: null, addToCalendar: false }; } 
...
const HabitActionModal = ({ isVisible, onClose, onEdit, onDelete, styles, theme }) => (
    <Modal visible={isVisible} transparent animationType="fade" onRequestClose={onClose}>
        <Pressable style={styles.modalBackdrop} onPress={onClose}>
            <View style={[styles.modalContainer, styles.actionModal]}>
                <TouchableOpacity style={styles.actionModalButton} onPress={onEdit}><Ionicons name="create-outline" size={22} color={theme.textPrimary} /><Text style={styles.actionModalButtonText}>Edit Habit</Text></TouchableOpacity>
                <TouchableOpacity style={styles.actionModalButton} onPress={onDelete}><Ionicons name="trash-outline" size={22} color={styles.destructive.color} /><Text style={[styles.actionModalButtonText, styles.destructive]}>Delete Habit</Text></TouchableOpacity>
            </View>
        </Pressable>
    </Modal>
);
...
const GenericHabitWidget = ({ habit, onUpdate, onOpenMenu, styles, theme }) => {
    ...
    switch (habit.type) {
        case 'daily_boolean':
            ...
            mainAction = (<TouchableOpacity style={styles.actionArea} onPress={() => onUpdate(habit.id)}><View style={isCompleted ? [styles.checkboxCompleted, { backgroundColor: habitColor }] : [styles.checkbox, { borderColor: habitColor }]}>{isCompleted && <Ionicons name="checkmark" size={20} color={onHabitColor} />}</View></TouchableOpacity>);
            break;
        case 'weekly_frequency':
            ...
            const buttonStyle = hasProgressToday ? { backgroundColor: habitColor } : { backgroundColor: tinycolor(habitColor).setAlpha(0.15).toRgbString() }; 
            const textStyle = hasProgressToday ? { color: onHabitColor } : { color: habitColor };
        ...
        case 'quit_habit':
            ...
            mainAction = <TouchableOpacity style={styles.plusButton} onPress={() => onUpdate(habit.id)}><Ionicons name="add" size={24} color={styles.destructive.color} /></TouchableOpacity>;
            break;
    }
    const cardStyle = habit.type !== 'quit_habit' ? { borderLeftColor: habitColor, borderLeftWidth: 5 } : (isCompleted && styles.lapsedCard);
    return (
        <Card style={[styles.card, cardStyle, isCompleted && habit.type !== 'quit_habit' && styles.completedCard]}>
        ...
        <SafeAreaView style={styles.screen}>
            ...
            <ScrollView contentContainerStyle={styles.container}>
                ...
                <Animated.View style={animatedListStyle}>
                    ...
                    {habits.map(habit => ( <GenericHabitWidget key={habit.id} habit={habit} onUpdate={handleUpdateProgress} onOpenMenu={openActionMenu} styles={styles} theme={theme} /> ))}
                </Animated.View>
...
const getDynamicStyles = (theme) => {
    ...
    return StyleSheet.create({
      ...
      plusButton: { width: 44, height: 44, borderRadius: 22, backgroundColor: destructiveBgColor, justifyContent: 'center', alignItems: 'center' },
