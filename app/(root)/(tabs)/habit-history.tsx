...
const HabitHistoryDetails = ({ habit, onClearHistory }) => {
  if (!habit) {
    return (
  ...
  const markedDates = useMemo(() => {
    ...
    for (const date in habit.history) {
        ...
        if (habit.type === 'quit_habit' && (dayData.progress || 0) > 0) {
            marks[date] = { selected: true, selectedColor: COLORS.destructive };
  ...
  const calendarTheme = {
      backgroundColor: COLORS.surface, calendarBackground: COLORS.surface, textSectionTitleColor: COLORS.textSecondary,
      selectedDayBackgroundColor: COLORS.primary, selectedDayTextColor: COLORS.white, todayTextColor: COLORS.primary,
      dayTextColor: COLORS.textPrimary, textDisabledColor: '#d9e1e8', arrowColor: COLORS.primary, monthTextColor: COLORS.textPrimary,
  ...
  return (
    <View style={styles.detailsContainer}>
      ...
      <View style={styles.statsRow}>
        <StatCard label={rateLabel} value={`${completionRate}%`} icon="checkmark-done-circle-outline" />
        <StatCard label="Current Streak" value={`${current} days`} icon="flame-outline" />
        <StatCard label="Longest Streak" value={`${longest} days`} icon="trophy-outline" />
      </View>
...
export default function HabitHistoryPage() {
  ...
  const renderHabitChip = ({ item: habit }) => {
    ...
    const selectedStyle = isSelected ? { backgroundColor: habit.color || COLORS.primary } : {};
    return (
        <TouchableOpacity style={[ styles.habitChip, selectedStyle ]} onPress={() => setSelectedHabit(habit)}>
          ...
          <Text style={[styles.habitChipText, isSelected && { color: COLORS.white }]}>{habit.name}</Text>
        </TouchableOpacity>
    );
  };
    ...
    <SafeAreaView style={styles.screen}>
      <Stack.Screen options={{ 
        ...
        headerLeft: () => (
            ...
            <TouchableOpacity onPress={() => router.replace('/(root)/(tabs)/habit-tracker')} style={{ marginLeft: 16, padding: 4 }}>
      ...
      <View style={styles.container}>
        ...
        <ScrollView>
            <HabitHistoryDetails habit={selectedHabit} onClearHistory={handleClearHistory} />
        </ScrollView>
      </View>
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  container: { flex: 1 },
  habitSelectorContainer: { borderBottomWidth: 1, borderBottomColor: '#E5E7EB', backgroundColor: COLORS.surface },
  noHabitsContainer: { paddingVertical: 40, alignItems: 'center', justifyContent: 'center', gap: 16 },
  noHabitsMessage: { fontSize: 16, color: COLORS.textSecondary },
  habitChip: { flexDirection: 'row', alignItems: 'center', backgroundColor: COLORS.surface, paddingHorizontal: 16, paddingVertical: 10, borderRadius: 20, marginRight: 12, borderWidth: 1, borderColor: '#E5E7EB' },
  habitChipIcon: { fontSize: 16, marginRight: 8 },
  habitChipText: { fontSize: 16, fontWeight: '600', color: COLORS.textPrimary },
  detailsContainer: { padding: 16, gap: 24 },
  noHabitText: { textAlign: 'center', fontSize: 16, color: COLORS.textSecondary, marginTop: 40 },
  calendarWrapper: { backgroundColor: COLORS.surface, borderRadius: 16, overflow: 'hidden', paddingBottom: 8 },
  statsRow: { flexDirection: 'row', justifyContent: 'space-around', gap: 12 },
  statCard: { alignItems: 'center', backgroundColor: COLORS.surface, padding: 16, borderRadius: 16, flex: 1, gap: 8, shadowColor: "#9FB1C4", shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.08, shadowRadius: 12, elevation: 3 },
  ...
  clearButton: { flexDirection: 'row', gap: 8, backgroundColor: COLORS.destructiveBg, padding: 15, borderRadius: 12, alignItems: 'center', justifyContent: 'center' },
  clearButtonText: { color: COLORS.destructive, fontSize: 16, fontWeight: '600' }
});